...
function loadWidget(config) {
	...
	let userAction = false,
		userActionTimer,
		messageTimer,
		messageArray = ["好久不见，甚是想念……", "好没理人家了呀，嘤嘤嘤～", "快来逗我玩吧，主人！", "拿小拳拳锤你胸口！"];
	...
	(function registerEventListener() {
		...
		devtools.toString = () => {
			showMessage("你打开控制台，是想要看看我的小秘密吗？", 6000, 9);
		};
		window.addEventListener("copy", () => {
			showMessage("你都复制了些什么呀，快来围观！", 6000, 9);
		});
	...
	(function initModel() {
		...
		fetch(waifuPath)// 获取waifu-tips.json的文案
			...
			.then(result => {
				...
				window.addEventListener("click", event => { //点击模型随机showHitokoto接口文案
					showHitokoto();
				...
				result.seasons.forEach(tips => {//节日匹配
					const now = new Date(),
	...
	// 获取model_list模型列表，可以删除自己不用的模型
	async function loadModelList() {
		const response = await fetch(`${cdnPath}model_list.json`);
		modelList = await response.json();
		console.debug(`${cdnPath}model_list.json`);
	...
	async function loadModel(modelId, modelTexturesId, message) {
		...
		if (useCDN) {
			...
			console.debug(`Live2D 模型 ${modelId}-${modelTexturesId} 加载完成`);
		}
	}
	// 随机加载模型，配置CDN则执行前者
...
function initWidget(config, apiPath) {
	...
	if (localStorage.getItem("waifu-display") && Date.now() - localStorage.getItem("waifu-display") <= 86400000) { //本地记录小于24hours则视为第一次使用
